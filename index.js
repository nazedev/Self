...
╚═══❏ *NAZE - SELF* ❏══❍`
	...
	case 'self':
     ...
     case 'memeindo':  
					...
					naze.sendMessage(from, buffer, image, {quoted: mek, caption: '*MEME🗿*'})
					break 
	     case 'quran':  
				    if (!mek.key.fromMe) return fakestatus('SELF-BOT')
			        if (!isGroup) return reply(mess.only.group)
					quran = await fetchJson(`https://api.zeks.xyz/api/randomquran?apikey=apivinz`)
					buffer = await getBuffer(quran.result)
					naze.sendMessage(from, buffer, text, {quoted: mek})
					break 
	     case 'quote':
				    if (!mek.key.fromMe) return fakestatus('SELF-BOT')
			        if (!isGroup) return reply(mess.only.group)
					quot = await fetchJson(`https://api.zeks.xyz/api/quote?apikey=apivinz`)
					buffer = await getBuffer(quot.result)
					naze.sendMessage(from, buffer, text, {quoted: mek})
					break 
 	case 'hidetag':
			if (!mek.key.fromMe) return fakestatus('SELF-BOT')
